...
//        operationButton.isEnabled = true
    ...
    func chooseImageAllert() {
        ...
        let actionPresentGallery = UIAlertAction(title: "Установить из галлереи", style: .default) { (_: UIAlertAction) in
            self.presentGallery()
        }

        let actionGetCamera = UIAlertAction(title: "Сделать фото", style: .default) { (_: UIAlertAction) in
            self.getCamera()
        }
